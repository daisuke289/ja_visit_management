...
RSpec.describe DiagnosisPolicy, type: :policy do
  ...
  describe "支店長" do
    let(:policy) { described_class.new(user, diagnosis) }
    ...
    context "他支店の顧客" do
      ...
      let(:other_user) { create(:user, branch: other_branch) }
      let(:other_diagnosis) { create(:diagnosis, customer: other_customer, user: other_user) }
      let(:policy) { described_class.new(user, other_diagnosis) }
  ...
  describe "本店管理者" do
    ...
    let(:policy) { described_class.new(admin, diagnosis) }
